...
<body>
	...
	<div class="center">
		...
		<caption>
			<h2>
				<c:if test="${employee.id != null}">Edit Task </c:if>
				<c:if test="${employee.id == null}">Add New Task</c:if>
			</h2>
		</caption>
		<c:if test="${employee.id != null}">
			<input type="hidden" name="idemployee" value="<c:out value='${employee.id}' />" />
		</c:if>
		<div class="txt_field">
			<input type="text" value="<c:out value='${employee.fullname}' />"
				name="fullname" id="fullname" required> 
				<label>Full Name:</label>
		</div>
		<div class="txt_field">
			<input type="email" value="<c:out value='${employee.email}' />"
				name="email" id="email" required> <label>Email:</label>
		</div>
		<div class="txt_field">
			<input type="text" value="<c:out value='${employee.departement}' />"
				name="departement" id="departement" required> <label>Departement:</label>
		</div>
